...
#### 5. 型エラー：文字列×整数の演算（2025-11-01解決）
- **症状**: `Unsupported operand types: string * int` エラーが発生
- **影響**: DailyClosing（日次精算）の計算処理が失敗
- **根本原因**:
  - フォーム入力値（price、quantity）が**文字列**として保存される
  - 過去の修正（a69b6089）で `?? 0` を追加したが、**型変換を忘れた**
  - `"100" ?? 0` は `"100"`（文字列のまま）になり、`"100" * 1` で型エラー
  - **PHP 8では型の厳格性が高まり、string * int はエラーになる**
- **発生箇所**:
  - `DailyClosing.php`: 5箇所（737, 741, 746, 945, 963行目）
  - `SalePostingService.php`: 2箇所（298, 304行目）
- **解決策**: `floatval()` と `intval()` で明示的に型変換
  ```php
  // 修正前（不完全）
  $total = ($price ?? 0) * ($quantity ?? 1);

  // 修正後（正しい）
  $total = floatval($price ?? 0) * intval($quantity ?? 1);
  ```
- **教訓**:
  - **`??` 演算子は null チェックのみで、型変換はしない**
  - フォーム入力値は常に文字列として扱う
  - 演算前に必ず型変換（floatval/intval）を行う
  - 過去の修正が不完全な場合、同じエラーが再発する
  - 全体検索で同じパターンを一括修正する

#### 6. 毎朝の500エラー（ログファイル権限問題）（2025-11-01解決）
...
- **予防的ハードニング**（2025-11-01追加）:
  - `config/logging.php` に `permission => 0664` を追加（新規ログファイルの権限を明示的に指定）
  - 日次ヘルスチェック追加（`health:check` コマンドを毎日00:05に実行）
    - ログファイル所有者の自動確認
    - DB・キュー接続確認
    - 異常時はログに警告を記録
  - タイムゾーン統一確認ワークフロー（OS/PHP-FPM/Laravel）
  - ACL/setgid 環境適合性確認ワークフロー
