<x-filament-widgets::widget>
    ...
    <x-filament::card>
        ...
        <style>
            ...
            .booking-block.span-1 { width: calc(var(--cell-width) * 1); }
            .booking-block.span-2 { width: calc(var(--cell-width) * 2); }
            .booking-block.span-3 { width: calc(var(--cell-width) * 3); }
            .booking-block.span-4 { width: calc(var(--cell-width) * 4); }
            .booking-block.span-5 { width: calc(var(--cell-width) * 5); }
            .booking-block.span-6 { width: calc(var(--cell-width) * 6); }
            ...
            div.booking-block.recently-created div.booking-name {
                animation: name-blink 0.8s ease-in-out infinite !important;
                color: #2563eb !important;
                font-weight: bold !important;
            }
            /* フォールバック：点滅が効かない場合のパルスエフェクト */
        ...
        <div class="overflow-x-auto" style="position: relative;">
            ...
            @if(!empty($timelineData))
                @php
                    // セル幅を予約枠の長さに比例させる（調整係数: 1.567）
                    // 15分間隔の場合: 15 * 1.567 = 23.5px
                    $slotDuration = $timelineData['slotDuration'] ?? 30;
                    $cellWidth = $slotDuration * 1.567;
                @endphp
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <p class="text-xs text-gray-500 mb-1">LINE連携</p>
                                <p class="text-base text-gray-700">
                                    @if($selectedReservation->customer->line_user_id)
                                        ...
                                        @if($selectedReservation->customer->line_linked_at)
                                            <span class="block text-xs text-gray-500 mt-1">
                                                {{ \Carbon\Carbon::parse($selectedReservation->customer->line_linked_at)->format('Y/m/d') }}
                                            </span>
                                        @endif
                                    @else
